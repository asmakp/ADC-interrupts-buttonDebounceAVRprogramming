...
#include "timer.h"
#define LED_PIN_PWM 6

void timer_init() //timer 0
{

	//Oscars tips to clear previous settings
	TCCR0A = 0;
	TCCR0B = 0;
	...
	TCCR0B &= ~(1 << WGM02);			  //select CTC mode,set WGM02 to 0, normal opration, OC0A(PD6) is disconnected
										  // TCCR0B = (1 << WGM02); //testing to see if pwm func works
...
void timer_intitPWM() //timer zero in pwm mode
{
	DDRD |= (1 << LED_PIN_PWM);
	...
	TCCR0B &= ~(1 << WGM02);
	//TCCR0B = (1 << WGM02);  // Fast PWM OCRA BOTTOM
...
void timer2_init() //timer two
{
	...
	TIMSK2 = (1 << OCIE2A); //Interrupt Mask Register, Output Compare A Match Interrupt Enable
